...
export function applyAdapter(
  ...
  for (let i = 0; i < chain.length; i++) {
    ...
    const target = isLast ? output : createMatrix('rgb');

    chain[i](currentInput, target);

    if (currentInput !== input && currentInput !== output) {
      dropMatrix(currentInput);
    }
    currentInput = target;
  }
}
...
export function convertColor<S extends ColorSpace, X extends ColorSpace>(
  ...
  const targetHub = NATIVE_HUB[to] ?? (to as 'xyz50' | 'xyz65');
  ...
  if (sourceHub !== targetHub) {
    if (sourceHub === 'xyz65') xyz65ToXyz50(output, output);
    else xyz50ToXyz65(output, output);
  }
