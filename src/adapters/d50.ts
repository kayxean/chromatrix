...
// Pre-calculated white point and its inverse for D50
...
/**
 * Converts XYZ (D50) to CIELAB
 */
export function xyz50ToLab(input: ColorArray, output: ColorArray): void {
  const xr = input[0] * INV_D50_X;
  const yr = input[1]; // White Y is 1.0, so division is skipped
  const zr = input[2] * INV_D50_Z;

  // Inlining lab_f to eliminate call overhead and allow better register allocation
