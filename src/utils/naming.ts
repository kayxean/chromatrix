...
// CSS Level 4 named colors (140 colors)
...
/**
 * Find the closest CSS named color to the given color.
 * Returns the color name and the perceptual distance.
 */
...
/**
 * Get the exact CSS color name if it matches, otherwise return null.
 * Uses a small tolerance for floating point comparison.
 */
export function getExactName(color: Color, tolerance = 0.001): string | null {
  const { name, distance } = findClosestName(color);
  return distance < tolerance ? name : null;
}

/**
 * Get all CSS color names sorted by distance from the given color.
...
/**
 * Parse a CSS color name into a Color object.
 * Returns null if the name is not recognized.
 */
export function parseColorName(name: string): Color | null {
  const rgb = CSS_COLORS[name.toLowerCase()];
  if (!rgb) return null;
